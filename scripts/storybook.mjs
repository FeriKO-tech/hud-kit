...
const workspaceRoot = repoRoot.includes('#') ? await createTemporaryWorkspace(repoRoot, mode) : repoRoot;
...
try {
  ...
  if (mode === 'dev') {
    await run(['--filter', 'storybook', 'dev'], workspaceRoot);
  } else {
...
async function createTemporaryWorkspace(sourceRoot, currentMode) {
  ...
  const targetRoot = join(tmpdir(), `hud-kit-storybook-${currentMode}-${digest}`);
